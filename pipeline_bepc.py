...
def list_uploads():
    client = storage.Client()
    blobs  = list(client.bucket(BUCKET_NAME).list_blobs(prefix="ingestion_bepc/"))
...
def run():
    ...
    for blob in uploads:
        ...
        if not analyse.get("est_educatif") or analyse.get("score_qualite", 0) < SCORE_MIN:
            ...
            continue
        matiere  = "ANGLAIS"  # force ANGLAIS pour ce lot mono-matiere
        serie    = "BEPC"  # BEPC sans series - force pour ce lot
        ...
        if integres % 5 == 0:
            save_database(data)
            print(f"  >>> Sauvegarde intermediaire: {integres} docs")
    if integres > 0:
        save_database(data)
